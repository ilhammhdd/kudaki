...
import (
	...
	"github.com/ilhammhdd/kudaki-event-payment-service/externals"
...
func main() {
	...
	wp.Worker <- new(externals.KudakiEventAdded)
	wp.Worker <- new(externals.RetrieveOrganizerTransaction)
	wp.Worker <- new(externals.PaymentRequestDoku)
